...
// Phones get a bottom sheet, desktop gets the anchored dropdown. A 96%-wide
// panel pinned to a screen corner is just a bad modal, so below `sm` the same
// markup slides up from the bottom edge where the thumb already is.
const panelClass =
    'fixed inset-x-0 bottom-0 z-50 rounded-t-3xl border-t p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] ' +
    'sm:absolute sm:inset-x-auto sm:right-0 sm:bottom-auto sm:mt-3 sm:w-96 sm:rounded-2xl sm:border sm:pb-4 ' +
    'bg-[#FFFFFF] border-[#E6E5E1] text-[#16181F]';

const iconButtonClass =
    'w-11 h-11 shrink-0 rounded-full border border-[#E6E5E1] flex items-center justify-center transition-all relative cursor-pointer bg-[#FFFFFF] text-[#16181F] hover:border-[#0B6BCB]/40';

function AfterLoginHeader({ searchQuery, setSearchQuery, onOpenReportModal, title = "Dashboard" }) {
    ...
    return (
        <header className="flex items-center justify-between gap-3 mb-5 sm:mb-8 relative" ref={popoverRef}>
            {/* Scrim behind the mobile bottom sheet only */}
            {activePopover && (
                <div className="sm:hidden fixed inset-0 bg-[#16181F]/30 z-40" onClick={() => setActivePopover(null)} />
            )}

            <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold tracking-tight text-[#16181F] truncate">{title}</h1>

            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                {/* Pill Search Field — hidden on phones, where the header is one row */}
                ...
                {/* Create — phones use the bottom bar's centre button instead */}
                <button
                    className="hidden md:flex shrink-0 min-h-[44px] bg-[#0B6BCB] text-white px-5 py-2.5 rounded-full text-sm font-extrabold transition-all active:scale-95 items-center gap-1.5 cursor-pointer hover:opacity-90"
                ...
                <div className="shrink-0">
                    ...
                    {activePopover === 'notifications' && (
                        <div className={panelClass}>
                            ...
                            <div className="max-h-[50vh] sm:max-h-72 overflow-y-auto space-y-3 pt-3">
                                {notifications.map((n) => (
                                    <div key={n.id} className={`p-2.5 rounded-xl border text-xs space-y-1 transition-colors ${n.unread ? 'bg-[#0B6BCB]/10 border-[#0B6BCB]/30' : 'bg-[#FFFFFF] border-[#E6E5E1]'}`}>
                                        <div className="flex justify-between items-center gap-2 font-bold text-[#16181F]">
                                            <span>{n.title}</span>
                                            <span className="text-xs sm:text-[10px] text-[#6B7280] shrink-0">{n.time}</span>
                                        </div>
                                ...
                                {messages.map((m) => (
                                    <div key={m.id} className="p-2.5 rounded-xl border flex gap-3 items-center cursor-pointer hover:opacity-90 bg-[#FFFFFF] border-[#E6E5E1]">
                                        ...
                                        <div className="flex-1 overflow-hidden">
                                            <div className="flex justify-between items-center gap-2 text-xs font-bold text-[#16181F]">
                                                <span className="truncate">{m.sender}</span>
                                                <span className="text-xs sm:text-[10px] text-[#6B7280] shrink-0">{m.time}</span>
                                            </div>
                    ...
                    {activePopover === 'profile' && (
                        <div className={`${panelClass} sm:w-64`}>
                            <div className="flex items-center gap-3 pb-3 border-b border-[#E6E5E1]">
                                <img
                                    className="w-11 h-11 rounded-full object-cover border border-[#E6E5E1] shrink-0"
                                    src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80"
                                    alt=""
                                />
                            ...
                            <div className="pt-3 space-y-1">
                                {currentUser?.role === 'superadmin' ? (
                                    <>
                                        <button
                                            ...
                                            className="w-full text-left px-3 min-h-[44px] rounded-xl text-sm sm:text-xs font-semibold flex items-center gap-2 cursor-pointer hover:bg-[#F4F3F1] text-[#16181F]"
                                        >
                                ...
                                <button
                                    ...
                                    className="w-full text-left px-3 min-h-[44px] rounded-xl text-sm sm:text-xs font-bold text-[#B42318] hover:bg-[#B42318]/10 flex items-center gap-2 cursor-pointer"
                                >
